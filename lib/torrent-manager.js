...
class PeerWire {
  ...
  _reqNext() {
    ...
    const reqPieceLength =
      index === this.pieceHashes.length - 1 ? this.lastPieceLength : this.pieceLength;
...
class TorrentSession extends EventEmitter {
  constructor({
    ...
    this.store =
      files.length === 1
        ? new SignleFileStore(outputPath, pieceLength, lastPieceLength, pieceHashes.length)
        : new MultiFileStore(outputPath, pieceLength, files);
    ...
    this.emit(
      'logging',
      `[session-${id}] infoHash=${infoHash.slice(0, 8)}... pieces=${pieceHashes.length} mode=${mode}`
    );
  ...
  _setUpTrackerEventListener() {
    this.client.on('warning', (err) => {
      ...
      this.emit(
        'logging',
        `[session-${this.id} tracker error] EVENT=${err.event} URL=${err.url} MESSAGE=${err.message}`
      );
    });
  ...
  addWire(wire, socket, peerId = null) {
    ...
    socket.on('error', (err) => {
      ...
      this.emit('logging', `[session-${this.id}] socket error: ${err.message}`);
      this.peerWires.delete(pw);
      this.uniquePeers.delete(`${pw.socket.remoteAddress}:${pw.socket.remotePort}`);
    });
    return pw;
  ...
  connect(ip, port) {
    ...
    socket.on('error', (err) =>
      // console.error(`[session-${this.id}] connect error ${ip}:${port} message=${err.message}`, err.message)
      this.emit(
        'logging',
        `[session-${this.id}] connect error ${ip}:${port} message=${err.message}`
      )
    );
  }
...
class TorrentManager {
  constructor({
    ...
    this._tcpServer.listen(this.listenPort, () => {});
  }
  /**
   ...
   */
  add({ parsed, outputPath, stratery = 'sequential', mode = 'leeching' }) {
