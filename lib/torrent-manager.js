...
class PeerWire {
    ...
    _reqNext() {
        ...
        const reqPieceLength = this.pieceHashes.length - 1 ? this.lastPieceLength : this.pieceLength;
...
class TorrentSession extends EventEmitter {
    ...
    addWire(wire, socket, peerId = null) {
        ...
        socket.on('close', () => this.peerWires.delete(pw))
        socket.on('error', (err) => {
    ...
    onPieceVerified(index, writeBuf) {
        ...
        // TODO this.emit('piece', (sessionId, index))
