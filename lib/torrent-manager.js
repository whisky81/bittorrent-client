...
class TorrentManager {
    ...
    connect(sessionId, ip, port) {
        ...
        const sessionCt = this.uniquePeers.get(sessionId);
        if (sessionCt != null && sessionCt.has(`${ip}:${port}`)) {
            return 
        }
        if (sessionCt == null) this.uniquePeers.set(sessionId, new Set());
        this.uniquePeers.get(sessionId).add(`${ip}:${port}`)
