...
class MultiFileStore {
   ...
   */
  constructor(outputPath, pieceLength, files) {
    ...
    try {
      ...
      for (const file of this.files) {
        ...
        const loc = outputPath + path.sep + file.name;
        const fd = fs.openSync(loc, fs.existsSync(loc) ? 'r+' : 'w+');
        this.fds.push(fd);
      }
    } catch (err) {
      // Clean up already opened fds
      this.close();
      throw new Error(`Failed to open files: ${err.message}`, { cause: err });
    }
  }
  ...
  _readAt(torrentOffset, length) {
    ...
    if (remaining > 0) {
      throw new Error(
        `Read beyond torrent boundaries: requested ${length}, got ${length - remaining}`
      );
    }
    return buf;
  }
  ...
  _writeAt(torrentOffset, data) {
    ...
    if (remaining > 0) {
      throw new Error(
        `Write beyond torrent boundaries: attempted ${data.length}, written ${data.length - remaining}`
      );
    }
  }
    ...
    if (this.fds) {
      for (const fd of this.fds) {
        try {
          ...
          // ignore
          console.log('[close fd error] ', err.message);
        }
      }
      this.fds = [];
    }
