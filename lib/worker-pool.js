...
class WorkerPool {
  ...
  constructor(workerPath, size, onResult, onError = null) {
    ...
    for (let id = 0; id < size; id++) {
      ...
      try {
        ...
        throw new Error(`Failed to create worker ${id}: ${err.message}`, { cause: err });
      }
      ...
      w.worker.on('exit', (code) => {
        if (code !== 0 && !this.isTerminated) {
          console.log(`[worker-${id} exit] code=${code}`);
  ...
  push(index, arrayBuf, writeBuf, expectedHash, sessionId) {
    ...
    const w = this.workers.reduce((a, b) => (a.queue.length <= b.queue.length ? a : b));
