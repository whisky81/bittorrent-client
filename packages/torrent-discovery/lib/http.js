...
export function announce(client, url, opts) {
  ...
  const req = transport.get(fullUrl, (res) => {
      ...
      try {
        ...
        if (decoded['failure reason']) {
          return client.emit(
            'warning',
            new TrackerError(
              Buffer.from(decoded['failure reason']).toString(),
              opts.event,
              'http',
              url,
              'HTTP_TRACKER_FAILURE_REASON'
            )
          );
        }
        ...
        client.emit(
          'warning',
          new TrackerError(err.message, opts.event, 'http', url, 'UNKNOWN_ERROR')
        );
      }
    });
    ...
    res.on('error', (err) =>
      client.emit(
        'warning',
        new TrackerError(err.message, opts.event, 'http', url, 'RESPONSE_ERROR')
      )
    );
  });

  req.setTimeout(REQUEST_TIMEOUT, () =>
    req.destroy(new TrackerError('timeout', opts.event, 'http', url, 'TIMEOUT'))
  );
