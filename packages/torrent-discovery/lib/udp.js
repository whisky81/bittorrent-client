...
export function announce(client, url, opts) {
  ...
  if (!port)
    return client.emit(
      'warning',
      new TrackerError('missing a port in tracker url', opts.event, 'udp', url, 'MISSING_PORT')
    );
  ...
  function cleanup() {
    ...
    try {
      ...
      console.log('[udp cleanup error] ', err.message);
    }
    socket = null;
  }
  ...
  function onMessage(msg) {
    ...
    switch (action) {
      case ACTIONS.CONNECT: {
        if (msg.length < 16)
          return onError(
            new TrackerError('invalid connect response', opts.event, 'udp', url, 'RESPONSE_ERROR')
          );
      ...
      case ACTIONS.ANNOUNCE: {
        ...
        if (msg.length < 20)
          return client.emit(
            'warning',
            new TrackerError('invalid announce response', opts.event, 'udp', url, 'RESPONSE_ERROR')
          );
      ...
      case ACTIONS.ERROR: {
        ...
        client.emit(
          'warning',
          new TrackerError(msg.slice(8).toString(), opts.event, 'udp', url, 'UNKNOWN_ERROR')
        );
        break;
      }
    }
  ...
  socket.bind(() => {
    timer = setTimeout(
      () => onError(new TrackerError('timeout', opts.event, 'udp', url, 'TIMEOUT')),
      REQUEST_TIMEOUT
    );
